...
from schemas.passage import SinglePassageRequest, SinglePassageResponse
...
async def create_single_passage(request: SinglePassageRequest) -> SinglePassageResponse:
    try:
        ...
        passage_example = get_passage_examples(request.type_passage)
        logger.debug(f"지문 예시 불러오기:{passage_example[:25]}...")

        if request.type_structure:
            passage_structure = get_passage_structures(request.type_structure)
            logger.debug(f"지문 구조 불러오기:{passage_structure[:25]}...")
            passage_structure_section = f"""## 지문 구조 설계 : {request.type_structure}
