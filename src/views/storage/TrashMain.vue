<template>
    <div class="hidden md:block flex flex-col gap-3 mx-auto p-4 sm:p-8 w-full">
        ...
        <div class="flex items-start gap-3 mb-2">
            <p class="text-xl sm:text-2xl font-bold text-black">휴지통</p>
        </div>
        ...
        <!-- Table Section -->
        <div class="w-full bg-white rounded-xl shadow-sm overflow-x-auto">
            <div class="w-full overflow-x-auto">
                    ...
                    >
                        ...
                        <tbody class="divide-y text-[14px] sm:text-[16px]">
                            ...
                            >
                                ...
                                <td
                                    class="px-4 py-2 text-[#424242] text-center"
                                >
                                    {{ item.PAS_DATE }}
                                </td>
                ...
                <template v-else>
                    <div
                        class="flex justify-center items-center w-full h-[414px]"
                    >
        ...
        <!-- Pagination -->
        <div
            v-if="totalPages > 0 && computedWorkItems.length > 0"
    ...
    <div
        class="md:hidden w-full h-full overflow-scroll flex flex-col bg-gray-50"
    >
        <!-- Header -->
        <header
            ...
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-2">
                    ...
                    <div
                        v-if="
                            !isFilterOpen &&
                            (typeFilter || fieldFilter || searchQuery)
                        "
                    ...
                    >
                        ...
                        <template v-if="fieldFilter">
                            <span v-if="searchQuery" class="text-gray-400"
                                >·</span
                            >
        ...
        <div class="flex-1 bg-white">
            <div v-if="filteredItems.length === 0" class="p-8 text-center">
                <div class="text-gray-500 mb-4">삭제된 문서가 없습니다.</div>
            </div>
            <div v-else class="overflow-x-auto">
                <table class="w-full min-w-[600px] text-sm">
                    <thead
                        class="text-xs text-gray-600 border-b border-gray-100"
                    >
                        <tr>
                            <th class="px-4 py-3 text-left font-medium w-[40%]">
                                작업명
                            </th>
                            <th class="px-4 py-3 text-left font-medium w-[18%]">
                                분야
                            </th>
                            ...
                            <th
                                class="px-4 py-3 text-center font-medium w-[8%] whitespace-nowrap"
                            >
                                다운로드
                            </th>
                            <th
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        >
                            ...
                            <td
                                class="px-4 py-3 text-gray-600 truncate text-sm cursor-pointer"
                                @click="handleWorkItemClick(item)"
                            >
                                {{ item.PAS_TYPE }}
                            </td>
                            ...
                            <td
                                class="px-4 py-3 text-gray-600 whitespace-nowrap text-sm cursor-pointer"
                                @click="handleWorkItemClick(item)"
                            >
                                {{ item.PAS_DATE }}
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <button
                                    ...
                                    class="p-1.5 mx-auto block"
                                >
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        class="w-5 h-5"
                                        :class="
                                            item.PAS_IS_FAVORITE
                                                ? 'text-[#FF9F40]'
                                                : 'text-gray-300'
                                        "
                                        :fill="
...
// 최근 작업 내역 리스트 가져오기? 삭제된 항목 목록 조회?
const fetchWorkItems = async () => {
  try {
    const data = await apiGet('/api/pass/select/deletedList');
    ...
    console.error('휴지통 목록 조회 실패:', error.message);
  }
};
...
const finishEditing = async () => {
    if (editingIndex.value >= 0) {
        ...
        try{
            const data = await apiPut('/api/pass/update/title',{
...
const computedWorkItems = computed(() => {
    ...
    if (selectedField.value) {
        items = items.filter(
            (item) => item.PAS_KEYWORD === selectedField.value
        );
    }
    ...
    // 검색어 필터 적용
    items = advancedSearch(items, searchQuery.value);
...
// 검색 결과 유무 확인을 위한 computed 속성
const hasSearchResults = computed(() => {
    return searchQuery.value && computedWorkItems.value.length > 0;
});
...
const confirmDelete = async () => {
    try {
        ...
        const data = await apiPut('/api/pass/remove/each',{pasCodeList: selectedPasCodes});
...
const filteredItems = computed(() => {
    ...
    if (typeFilter.value) {
        items = items.filter(
            (item) => item.PAS_IS_GENERATED === typeFilter.value
        );
    }
