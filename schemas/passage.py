...
class SinglePassageRequest(BaseModel):
    ...
    keyword: List[str] = Field(..., description="지문에 포함할 제재 키워드 목록", example=["인공지능", "머신러닝", "딥러닝"])
...
class ReadingPassageRequest(BaseModel):
    keyword: List[str] = Field(..., description="지문에 포함할 제재 키워드 목록", example=["독서", "공감적 읽기", "가을"])
    requirement: Optional[str] = Field(None, description="추가 요청 사항 (선택 사항)", example="고3 수준으로 작성해주세요.")
...
class MultiplePassageRequest(BaseModel):
    ...
    first_keyword: List[str] = Field(..., description="(가) 지문 키워드 목록", example=["계층", "평등"])
    second_type_passage: PassageType = Field(..., description="(나) 지문의 분야", example="인문/예술/사회/과학/기술")
    second_keyword: List[str] = Field(..., description="(나) 지문 키워드 목록", example=["시장", "자본주의"])
